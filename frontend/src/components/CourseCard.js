...
const CourseCard = ({ course, userId }) => {
    ...
    return (
        <Link to={`/courses/${course._id}`} style={{ textDecoration: 'none' }}>
            <Card sx={{ height: '350px', margin: "8px", position: "relative" }}>
                <CardActionArea sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
                    ...
                    <CardContent sx={{ flex: "1" }}>
                        <Typography gutterBottom variant="h5" component="div" sx={{ lineHeight: '1.2', maxHeight: '80px', overflow: 'hidden' }}>
                            {course.title}
                        </Typography>
                    ...
                    <Box sx={{ position: 'absolute', bottom: '20px', left: '16px', right: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <Button onClick={handleClickBuy} variant="contained" color="primary" disabled={isProcessing || isCoursePurchased}>
                            {isProcessing ? 'Processing...' : isCoursePurchased ? 'Purchased' : 'Buy'}
                        </Button>
                    </Box>
                </CardActionArea>
            </Card>
